...
class RegisterLocationView(APIView):
    def get(self,request):
        call_model=RegisterLocation.objects.all()
        call_serializer=RegisterLocationSerializer(call_model,many=True,context={'request': request})
        return Response(data=call_serializer.data,status=status.HTTP_200_OK)


class ShopView(APIView):
    def get(self,request, location):
        call_model=Shop.objects.filter(location__city_name=location)
        call_serializer=Shop(call_model,many=True,context={'request': request})
        return Response(data=call_serializer.data,status=status.HTTP_200_OK)
    ...
    def get(self,request):
            try:
                ...
                if username is not None:
                    obj=Shop.objects.filter(user__username=username)
                    if obj is not None:
